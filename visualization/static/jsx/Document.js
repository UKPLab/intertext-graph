...
function Paragraph(props) {
    ...
    switch (props.ntype) {
        case 'article-title':
            content = <h5 dangerouslySetInnerHTML={{__html: props.content}} />
            break;
        case 'abstract':
            // Fall through
        case 'title':
            content = <h6 dangerouslySetInnerHTML={{__html: props.content}} />
            break;
        default:
    ...
    return (
        <li
            ...
            {content}
            <span className="badge badge-pill badge-primary mw-100 overflow-auto mr-1">{props.ntype}</span>
            {metadata.map((value, key) =>
                <span key={key} className="badge badge-pill badge-secondary mw-100 overflow-auto mr-1">{value}</span>
            )}
        </li>
    );
}
                    ...
                    <div className="col">
                        <div className="card bg-light shadow-sm mb-3">
                            <div className="card-body">
                                <h5 className="card-title">{this.props.serialized.meta.title}</h5>
                                ...
                                {this.props.serialized.meta.url &&
                                    <a
                                        ...
                                        <svg width="1em" height="1em" viewBox="0 0 16 16"
                                             className="bi bi-box-arrow-up-right" fill="currentColor"
